...
7. 配置文件字段说明，host为监听的域名，port为监听的端口，auth为网站访问是否需要登录，username为用户名，password为密码，auth_token为登录凭证（放在cookie），auth_cookie_key为cookie名字，auth_cookie_timeout为cookie有效期
